# 🫁 SEAS: Smart Endoscopic Autonomous System
**Powered by the B.A.S.H.A.R. Core Architecture**
...
**SEAS (Smart Endoscopic Autonomous System)** is a fully integrated ROS 2 simulation and control stack for a clinical robotic bronchoscope. It bridges the gap between pure robotics mathematics (*Modern Robotics*) and clinical application. 

This medical system is built on top of **B.A.S.H.A.R.** (*Bimodal Autonomous System for Handling and Articulated Robotics*), our universal core framework designed to provide custom kinematics, autonomous control, and computer vision pipelines for multi-link robotic chains.
...
* **Developer:** Zeyad A. A. Muhammed, Ahmed Etman, Ahmed AbdelMoety

---

## ✨ Key Features

### 🧮 1. The BASHAR Kinematics & Dynamics Engine (`broncho_kinematics.py`)
Built from scratch without relying on external libraries (like MoveIt or Pinocchio), directly implementing the mathematics of rigid-body motions adapted specifically for the 7-DOF endoscope:
...
* **Dynamics & Trajectories:** Recursive Newton-Euler inverse dynamics, mass matrices, and polynomial time-scaling.

### 🧠 2. Clinical AI Vision Pipeline (`vision_pipeline.py`)
A real-time diagnostic image processing node utilizing the BASHAR vision base logic. It reads from the Gazebo CMOS camera to classify and localize airway anomalies using multi-spectral HSV masking, morphological cleaning, and shape-feature extraction (solidity, circularity).
...
* **AutoPilot (Autonomous Navigation):** A computed-torque navigation system that inserts the scope at a constant velocity while continuously evaluating image moments to steer the distal tip safely through the bronchial tubes.
...
│   ├── broncho_kinematics.py    # BASHAR math engine adapted for 7-DOF
│   └── vision_pipeline.py       # Clinical OpenCV diagnostic logic
├── urdf/
...
```bash
cd ~/ros2_ws/src
git clone [https://github.com/YOUR_USERNAME/smart_endoscope.git](https://github.com/YOUR_USERNAME/smart_endoscope.git)
